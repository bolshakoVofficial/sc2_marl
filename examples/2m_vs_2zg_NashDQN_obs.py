...
# path to model file (ex: models_nash/128x64___.h5) OR None if new model
# LOAD_MODEL = "models_nash/64x64___444.79avg.h5"
LOAD_MODEL = None
...
class DQN:
    ...
    def next_predictions(self, batch):
        # from target net
        new_current_states = np.array([transition[3][self.agent_id] for transition in batch]) / 255
    ...
    def train(self, batch, nash_q_values, terminal_state):
        # from eval net
        current_states = np.array([transition[0][self.agent_id] for transition in batch]) / 255
        ...
        for index, (current_state, actions, rewards, new_state, new_actions, terminated) in enumerate(batch):
            ...
            X.append(current_state[self.agent_id])
            y.append(current_qs)
...
agents = [DQN() for i in range(n_agents)]
    ...
    while not terminated:
        ...
        for agent_id in range(n_agents):
            ...
            if np.random.random() > epsilon:
                q_values = agents[agent_id].get_q_values(obs[agent_id])
        ...
        replay_memory.update_replay_memory((obs, actions, rewards, env.get_obs(), new_avail_actions, terminated))
