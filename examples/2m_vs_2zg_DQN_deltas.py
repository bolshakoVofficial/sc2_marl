...
if not os.path.isdir('models_deltas'):
    os.makedirs('models_deltas')
...
for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit="episode"):
    ...
    while not terminated:
        ...
        network.update_replay_memory((state, joint_action, sum(rewards), env.get_state(), terminated))
    ...
    if not episode % AGGREGATE_STATS_EVERY:
        ...
        if average_reward >= new_average_reward:
            network.model.save(
                f"models_deltas/{MODEL_NAME}__{average_reward:_>7.2f}avg_{max_reward:_>7.2f}max_{min_reward:_>7.2f}min__{int(time.time())}.h5")
