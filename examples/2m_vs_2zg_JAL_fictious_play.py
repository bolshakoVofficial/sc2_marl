...
def learn(state, state2, reward, action):
    ...
    fictious_matrix[state, action] += 1
...
gamma = 0.95
EPSILON = 0.6
FICTIOUS_PLAY_PROB = 0.3
FP_START_DECAY = n_episodes // 4
FPP_DECAY_RATE = (1 / (n_episodes - FP_START_DECAY - n_episodes / 20)) * (1 - FICTIOUS_PLAY_PROB * 2)

q_table = np.zeros([n_states, n_actions * n_actions])
fictious_matrix = np.ones([n_states, n_actions * n_actions])
...
epsilon = EPSILON
fictious_play_prob = FICTIOUS_PLAY_PROB
...
for e in tqdm(range(1, n_episodes + 1), ascii=True, unit="episode"):
    ...
    # fictious_play_prob += (1 - fictious_play_prob) ** 0.1 / n_episodes
    if e > FP_START_DECAY:
        fictious_play_prob += FPP_DECAY_RATE
    ...
    if not e % 10:
        ...
        fict_values.append(fictious_play_prob)
    ...
    if (not e % 100) and (e != 0):
        with open(f'2v2_jal_fict_ep{e}.pkl', 'wb') as f:
...
with open("2v2_jal_fict_final.pkl", 'wb') as f:
    pickle.dump(q_table, f)
...
ax3.plot(x, mean_ep_reward)
ax3.set_title('Rewards')

ax4.plot(x, mean_total_steps)
ax4.set_title('Steps')
