...
MODEL_NAME = "64_no_Nash2aaaaaaaaaa"

# path to model file (ex: 128x64___.h5) OR None if new model
# LOAD_MODEL = "64__427.61avg__445.87max__398.92min__1590839584.h5"
LOAD_MODEL = None
...
EPS_WITH_EPSILON = 1200
EPS_WITH_ZERO_EPSILON = 1000
...
class DQN:
    ...
    def train(self, batch, nash_q_values, terminal_state):
        ...
        for index, (current_state, actions, rewards, new_state, new_actions, terminated) in enumerate(batch):
            if not terminated:
                max_future_q = np.max(nash_q_values[index][self.agent_id])
                new_q = rewards[self.agent_id] + GAMMA * max_future_q
            else:
...
for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit="episode"):
    ...
    while not terminated:
        ...
        if replay_memory.get_len() > MIN_REPLAY_MEMORY_SIZE:
            ...
            for agent in agents:
                agent.train(batch, future_qs, terminated)
