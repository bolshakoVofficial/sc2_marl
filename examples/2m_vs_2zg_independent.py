...
def learn(agent_id, state, state2, reward, action):
    q_table[state, action, agent_id] = q_table[state, action, agent_id] + alpha * \
                                       (reward + gamma * np.max(q_table[state2, :, agent_id]) - \
                                        q_table[state, action, agent_id])
...
q_table = np.zeros([n_states, n_actions, n_agents])
...
for e in tqdm(range(1, n_episodes + 1), ascii=True, unit="episode"):
    ...
    while not terminated:
        ...
        for agent_id in range(n_agents):
            learn(agent_id, states[agent_id], next_states[agent_id], rewards[agent_id], actions[agent_id])
    ...
    if (not e % 100) and (e != 0):
        with open(f'2v2_independent_ep{e}.pkl', 'wb') as f:
...
with open("2v2_independent_final.pkl", 'wb') as f:
    pickle.dump(q_table, f)
